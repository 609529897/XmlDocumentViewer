...
type AuthorXrefMap = {
  aff?: string[];       // affiliations
  fn?: string[];        // footnotes
  corresp?: string[];   // corresponding marks
...
export function parseAuthors(doc: Element, defaultLang: string): Author[] {
  ...
  return contribNodes.map((c) => {
    const idNode = c.querySelector("contrib-id[contrib-id-type='orcid']");
    const id = idNode?.textContent?.trim() || undefined;

    const surname = c.getElementsByTagName("surname")[0]?.textContent?.trim();
    const givenNames = c.getElementsByTagName("given-names")[0]?.textContent?.trim();
    ...
    const nameAltNodes = Array.from(c.getElementsByTagName("name-alternatives"));
    nameAltNodes.forEach((alt) => {
      Array.from(alt.children).forEach((child) => {
        ...
        if (child.tagName === "name") {
          const altSurname = child.getElementsByTagName("surname")[0]?.textContent?.trim();
          const altGiven = child.getElementsByTagName("given-names")[0]?.textContent?.trim();
          nameLangMap[childLang] = getFullName(altGiven, altSurname, childLang) || "";
    ...
    if (!fullName) {
      fullName = nameLangMap[defaultLang || "zh"] || Object.values(nameLangMap)[0];
    }
    ...
    xrefNodes.forEach((xref) => {
      ...
      (xrefs[refType] as string[]).push(rid || xref.textContent?.trim() || "");
    });
