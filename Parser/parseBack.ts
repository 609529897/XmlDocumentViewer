...
function parseAuthors(citationElement: Element): Author[] {
  ...
  // 查找所有可能的 person-group，优先 author 类型
  const personGroup = citationElement.querySelector('person-group[person-group-type="author"]');
  // if (!personGroup) {
  //   personGroup = citationElement.querySelector('person-group');
  // }
  
  if (!personGroup) return authors;
  
  // 同时处理 <name> 和 <string-name> 元素
  const nameElements = personGroup.querySelectorAll('name, string-name');
  
  nameElements.forEach(nameElement => {
    if (nameElement.tagName.toLowerCase() === 'name') {
      // 处理 <name> 元素（西文姓名）
    ...
    } else if (nameElement.tagName.toLowerCase() === 'string-name') {
      // 处理 <string-name> 元素（中文姓名）
      const fullName = nameElement.textContent?.trim() || '';
      if (fullName) {
        // 对于中文姓名，将全名同时放在 surname 和 fullName 中
        authors.push({ 
          surname: fullName, 
          givenNames: '', 
          fullName 
...
export function parseBack(back: Element) {
  ...
  const references: References[] = Array.from(
    ...
    // 首先尝试查找 citation-alternatives 中的 citation
    const citationAlternatives = ref.querySelector("citation-alternatives");
    let citation: Element | null = null;
    
    if (citationAlternatives) {
      citation = citationAlternatives.querySelector("mixed-citation, element-citation");
    }
  
    // 如果没有 citation-alternatives，则直接查找
    if (!citation) {
      citation = ref.querySelector("element-citation") || ref.querySelector("mixed-citation");
    }
