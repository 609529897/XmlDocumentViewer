...
export function xmlParser({ xml }: { xml: string }): XMLParserResult {
  ...
  const pub = articleMeta?.pubDates.find((item) => item.type === "ppub");
  const epub = articleMeta?.pubDates.find((item) => item.type === "epub");
  const received = articleMeta?.history.find((item) => item.type === 'received');
  const revised = articleMeta?.history.find((item) => item.type === 'revised');
  const accepted = articleMeta?.history.find((item) => item.type === 'accepted');
