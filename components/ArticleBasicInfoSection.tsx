  ...
  const articleInfoItems = [
    {
      ...
      value: data.journalId ? (
        <Link
          to={`/detail/${data.journalId}?type=journal`}
          target="_blank"
          onClick={(e) => {
            e.preventDefault();
    ...
    },
    {
      key: "年卷期",
      value:
        data.year && data.volume && data.issue
          ? `${data.year}年${data.volume}卷${data.issue}期`
          : "",
    },
    {
      key: "栏目",
      ...
      {articleInfoItems.map(
        (item) =>
          item.value && (
            ...
            >
              <span className="text-[var(--kx-text-1)] whitespace-nowrap">{item.key}：</span>
