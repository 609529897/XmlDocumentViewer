...
export function AuthorsAndInstitutions({ data }: { data: any }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {hasCount(authors) && <AuthorsSection authors={authors} />}
      {hasCount(affiliations) && <AffiliationsSection affiliations={affiliations} />}
      {hasCount(correspAuthors) && <CorrespondingSection corresp={correspAuthors} />}
      {hasCount(notes) && <NotesSection notes={notes} />}
      {hasCount(contributions) && <ContributionsSection contributions={contributions} />}
    </div>
